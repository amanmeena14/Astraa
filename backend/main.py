...
S3_BUCKET_NAME = os.getenv("S3_BUCKET_NAME", "syabuc")
...
async def generate_image(data: dict):
    """Generate an image based on a text prompt using Amazon Bedrock, encode it, and store in S3."""
    try:
        ...
        for img_b64 in images_b64:
            # Decode base64 to raw bytes and load into PIL
            raw_bytes = base64.b64decode(img_b64)
            pil_img = Image.open(io.BytesIO(raw_bytes))

            # Encode the prompt into the image via steganography
            stego_img = encode_text_in_image(pil_img, prompt)

            # Optional: verify embedding
            try:
                assert decode_text_from_image(stego_img) == prompt
            except AssertionError:
                logger.warning("Stego decode verification failed for prompt: %s", prompt)

            # Serialize the stego image back to PNG bytes
            buf = io.BytesIO()
            ...
            logger.info("Uploading stego image to S3 key: %s", image_key)
            ...
            logger.info("Stego image URL: %s", image_url)
